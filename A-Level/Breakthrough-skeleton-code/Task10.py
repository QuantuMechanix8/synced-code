...
class Breakthrough():
    ...
    def __GetCardFromDeck(self, CardChoice):
        if self.__Deck.GetNumberOfCards() > 0:
            ...
            # ADDITION START
            elif self.__Deck.GetCardDescriptionAt(0).strip().upper() == "G":
                CurrentCard = self.__Deck.RemoveCard(self.__Deck.GetCardNumberAt(0))
                print()
                print("You picked up a Genius Card")
                Chosen = False
                while not Chosen:
                    Choice = input("You can either (U)se the Card or (D)iscard it: ").strip().upper()
                    if Choice in {'U', 'D'}:
                        Chosen = True
                    else:
                ...
                if Choice == "U":
                    ...
                    while not Chosen:
                        print(self.__CurrentLock.GetLockDetails())
                        Choice = input(f"Which challenge would you like to solve (1 - {NumChallenges}): ")
                        print(self.__CurrentLock.GetLockDetails())
                        try:
                            Choice = int(Choice)
                            if (Choice >= 1 and Choice <= NumChallenges):
                                Chosen = True
                        except:
                            print("invalid challenge number - try again")
                    self.__CurrentLock.SetChallengeMet(Choice-1, True)
                    print("The challenge has been met")
                elif Choice == "D":
                    print("Genius card discarded")
            # END ADDITION
        while self.__Hand.GetNumberOfCards() < 5 and self.__Deck.GetNumberOfCards() > 0:
            ...
            # ADDITION START
            elif self.__Deck.GetCardDescriptionAt(0) == " G ":
                self.__MoveCard(self.__Deck, self.__Discard, self.__Deck.GetCardNumberAt(0))
    ...
    def AddGeniusCardToDeck(self):
        ...
        GenCard = GeniusCard()
        self.__Deck.AddCard(GenCard)
        print("added many genius cards to the deck")
    # ADDITION END
