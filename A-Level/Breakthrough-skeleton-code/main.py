...
#import Additions # my own file with added code and functions from practice questions
...
class Breakthrough():
        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    print()
                    print("Current score:", self.__Score, "\tDeck cards left:", len(self.__Deck._Cards))
                    ...
                    if MenuChoice == "D":
                        ...
                        if not self.__CurrentLock.GetPeekUsed():
                            print(f"final three cards in the deck are '{self.__Deck.Peek(3)}'")
                            self.__CurrentLock.SetPeekUsed()
                            print("you have now used your peek for this lock")
                        else:
                            print("you've already used your peek for this lock")
                    elif MenuChoice == "M":
                        self.__UseMulligan()
                    elif MenuChoice == "U":
                        ...
                        if DiscardOrPlay == "C":
                            # added code to allow a user to cancel the input
                            continue
                        elif DiscardOrPlay == "D":
    ...
    def __CheckIfLockChallengeMet(self):
        ...
        for Count in range(self.__Sequence.GetNumberOfCards() - 1, max(0, self.__Sequence.GetNumberOfCards() - 3) -1, -1):
            # iterates over last 3 cards in sequence backwards as challenge is 3 or less conditions
            if len(SequenceAsString) > 0:
                SequenceAsString = ", " + SequenceAsString # adds comma for formatting if the sequence isnt empty
    ...
    # CODE ADDITION
    def __UseMulligan(self):
        if self.__MulliganUsed:
            print("You have already used your mulligan this game")
            return
    # moving all cards back to deck
        HandCardNums = [card._CardNumber for card in self.__Hand._Cards]
        SeqCardNums = [card._CardNumber for card in self.__Sequence._Cards]
        DisCardNums = [card._CardNumber for card in self.__Discard._Cards]
        # moving hand to deck    
        for HandCard in HandCardNums:
            self.__MoveCard(self.__Hand, self.__Deck, HandCard)
        # moving sequence to deck
        for SeqCard in SeqCardNums:
            self.__MoveCard(self.__Sequence, self.__Deck, SeqCard)
        # moving discard to deck
        for DisCard in DisCardNums:
            self.__MoveCard(self.__Sequence, self.__Deck, DisCard)
        # shuffle deck
        self.__Deck.Shuffle()

    # redealing cards
        for i in range(5):
            newCard = self.__Deck.GetCardNumberAt(0)
            while newCard.GetDescription().lower() == "dif":
                self.__MoveCard(self.__Deck__, self.__Discard__, newCard.GetCardNumber())
                newCard = self.__Deck.GetCardNumberAt(0)
            self.__MoveCard(self.__Deck, self.__Hand__, newCard.GetCardNumber())
        # my code is TRASH!!
    # ADDITION END
    
    """def __GetCardChoice(self): # my own __GetCardChoice method to avoid error when selection out of range
        Choice = None
        validChoice = False
        max = self.__Hand.GetNumberOfCards()
        while not validChoice:
            try:
                Choice = int(input(f"Enter an integer between 1 and {max}: "))
            except:
                print(" - that wasn't an integer")
                Choice = None
            if Choice != None:
                if Choice<=max and Choice>=1:
                    validChoice = True
                    print("choice accepted\n")
                elif Choice>max:
                    print(f"choice too large - needs to be {max} or lower")
                elif Choice<1:
                    print(f"choice too small - needs to be above 1")"""

    def __GetDiscardOrPlayChoice(self):
        Choice = input("(D)iscard, (P)lay or (C)ancel?:> ").upper()
        return Choice

    def __GetChoice(self):
        print()
        if self.__MulliganUsed:
            Choice = input("(D)iscard inspect, (U)se card or (P)eek:> ").strip().upper()
        else:
            Choice = input("(D)iscard inspect, (U)se card, (P)eek or (M)ulligan:> ").strip().upper()
        return Choice
...
class Lock():
    def __init__(self):
        self._Challenges = []
        self._peekUsed = False # new attribute for a peek option for each lock
    ...
    def __ConvertLockToString(self):
        lockString = ""
        for challenge in self._Challenges:
            lockString.append(self.__ConvertConditionToString(challenge) + ";")
        lockString.remove(";", )
    ...
    def GetLockSolved(self):
        for C in self._Challenges:
            if not C.GetMet():
                return False
        return True
    # added accessors for new peekused attribute ####
...
class CardCollection():
    ...
    def Peek(self, quantity = 3):
        quantity = min(len(self._Cards), quantity)
        peekCards = self._Cards[0 : quantity]
        peekString = ""
        for index, card in enumerate(peekCards):
            peekString += card.GetDescription()
            peekString += ", "
        peekString = peekString[:-2] # removes final comma and space
        return peekString
