...
class Breakthrough():
        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    print(f"You have used {self.__CurrentLock.GetCardsUsedForLock()} cards on the current Lock")
                    ...
                    if MenuChoice == "D":
                        ...
                        if DiscardOrPlay == "D":
                            ...
                            # ADDITION START
                            self.__CurrentLock.UseCardOnLock()
                            # ADDITION END
    ...
    def __ProcessLockSolved(self):
        self.__Score += 10
        # ADDITION START
        BonusPoints = self.__CurrentLock.GetBonusPoints()
        print(f"You got an additional {BonusPoints} points because of how many cards it took to solve the lock")
        # ADDITION END
...
class Lock():
    def __init__(self):
        self._Challenges = []
        # ADDITION START
        self._CardsUsedForLock = 0

    def UseCardOnLock(self):
        self._CardsUsedForLock += 1

    def GetCardsUsedForLock(self):
        return self._CardsUsedForLock

    def GetBonusPoints(self):
        return max(0, 20 - self._CardsUsedForLock)
    # ADDITION END
